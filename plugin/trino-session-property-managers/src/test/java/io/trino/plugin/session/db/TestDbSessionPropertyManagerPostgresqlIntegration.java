...
/*********** Bloomberg customization — PostgreSQL integration test mirroring upstream's MySQL test ***********/
@TestInstance(PER_CLASS)
@Execution(SAME_THREAD)
public class TestDbSessionPropertyManagerPostgresqlIntegration
{
            ...
            throws IOException
    {
        try (Closer closer = Closer.create()) {
            ...
            closer.register(postgresContainer::close);
        }
        queryRunner = null;
    ...
    public void setupTest()
    {
        queryRunner.getCoordinator().getSessionPropertyDefaults()
                .setConfigurationManager("db-test", ImmutableMap.<String, String>builder()
                        .put("session-property-manager.db.url", postgresContainer.getJdbcUrl())
                        .put("session-property-manager.db.username", postgresContainer.getUsername())
                        .put("session-property-manager.db.password", postgresContainer.getPassword())
                        .buildOrThrow());

        jdbi = Jdbi.create(postgresContainer.getJdbcUrl(), postgresContainer.getUsername(), postgresContainer.getPassword());
        dao = jdbi
                .installPlugin(new SqlObjectPlugin())
                .onDemand(PostgresSessionPropertiesDao.class);
    }
