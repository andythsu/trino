...
import io.trino.plugin.trino.expression.RewriteCast;
import io.trino.plugin.trino.expression.RewriteDateConstant;
import io.trino.plugin.trino.expression.RewriteIntervalConstant;
import io.trino.plugin.trino.expression.RewriteTimestampConstant;
            ...
            extends BaseJdbcClient
{
    ...
    public Optional<ColumnMapping> toColumnMapping(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        switch (typeHandle.jdbcType()) {
            ...
            case Types.JAVA_OBJECT: return objectToTrinoType(jdbcTypeName);
            case Types.STRUCT: return rowToTrinoType(jdbcTypeName);
            default: {
                if (getUnsupportedTypeHandling(session) == CONVERT_TO_VARCHAR) {
                    return mapToUnboundedVarchar(typeHandle);
                }
            ...
            String jdbcTypeName)
    {
        ...
        if (objType instanceof MapType) {
            return Optional.of(mapColumnMapping((MapType) objType, quotedJdbcTypeName));
        }
        // By default, we are assuming the Objects are ROW types.
        return Optional.of(rowColumnMapping((RowType) objType, quotedJdbcTypeName));
    }
            ...
            String jdbcTypeName)
    {
        ...
        return Optional.of(rowColumnMapping(rowType, quotedJdbcTypeName));
    }
    ...
    private static ColumnMapping mapColumnMapping(MapType mapType, String baseElementJdbcTypeName)
    {
        return ColumnMapping.objectMapping(
                mapType,
                mapReadFunction(mapType),
                mapWriteFunction(mapType, baseElementJdbcTypeName));
    }
    ...
    private static ObjectWriteFunction mapWriteFunction(MapType elementType, String elementJdbcTypeName)
    {
    ...
    private static ColumnMapping rowColumnMapping(RowType rowType, String baseElementJdbcTypeName)
    {
        return ColumnMapping.objectMapping(
                rowType,
                rowReadFunction(rowType),
                rowWriteFunction(rowType, baseElementJdbcTypeName));
    }
    ...
    private static ObjectWriteFunction rowWriteFunction(Type elementType, String elementJdbcTypeName)
    {
...
//            Object objectValue = elementType.getObjectValue(sqlRow.getRawFieldBlocks(), )readNativeValue(elementType, block, index);
