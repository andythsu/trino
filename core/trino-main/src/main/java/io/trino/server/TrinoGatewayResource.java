...
public class TrinoGatewayResource
{
    ...
    @GET
    @Path("coordinatorMetrics")
    public ClusterMetrics coordinatorMetrics()
    {
        long totalFreeBytes = clusterMemoryManager.getCoordinatorsMemoryInfo()
                .values()
                .stream()
                .flatMap(Optional::stream)
                .map(MemoryInfo::getPool)
                .mapToLong(MemoryPoolInfo::getFreeBytes)
                .sum();
        double totalSystemLoad = clusterMemoryManager.getCoordinatorsMemoryInfo()
                .values()
                .stream()
                .flatMap(Optional::stream)
                .mapToDouble(MemoryInfo::getSystemCpuLoad)
